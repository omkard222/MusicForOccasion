...
FormtasticBootstrap::Inputs::RadioInput.module_eval do
  def bootstrap_wrapping(&block)
    form_group_wrapping do
      label_html <<
      '<br/>'.html_safe <<
      template.content_tag(:span, class: 'form-wrapper') do
        input_content(&block) << hint_html << error_html(:block)
      end
    end
  end

  def radio_wrapping(&block)
    template.content_tag(:label, template.capture(&block).html_safe, class: 'radio-inline')
  end
end

FormtasticBootstrap::Inputs::TextInput.module_eval do
